// these are all the constants that will be used
...
// these are polymorphic object types. enums are like grouped constants!
typedef enum {
PHYLIB_STILL_BALL = 0,
...
// class representing a vector in two dimensions
typedef struct {
    double x;
    double y;
} phylib_coord;

// classes representing objects on the table
// struct for still ball
typedef struct {
    unsigned char number;
    phylib_coord pos;
} phylib_still_ball;

// struct for rolling ball
typedef struct {
    unsigned char number;
    phylib_coord pos;
    phylib_coord vel;
...
//strcut for horizontal cushion
typedef struct {
    double y;
} phylib_hcushion;

// struct for vertical cushion
typedef struct {
    double x;
} phylib_vcushion;
...
// struct for object. links everything together
typedef struct {
    phylib_obj type;
    phylib_untyped obj;
} phylib_object;
...
// These are the prototypes for the functions in part 1!
...
// this is the function parameter for the function from assignment 2 part 1
